...
import SelectorGenes from '../models/selector-genes';
...
class SelectorCtrl extends BaseCtrl {
    model = Selector;
    geneModel = SelectorGenes;
    ...
    insertFile = async (req, res) => {
        try {
            ...
            if (!!req.files.file) {
                ...
                const selector = await this.model.findOne({
                    owner: decodedToken.user._id,
                    _id: req.body.selector
                });
                ...
                await fasta.obj(path.join(__dirname, '../uploads/') + uniqueFilename).on('data', async (s) => {
                    await new SelectorGenes({
                        name: s.id,
                        sequence: s.seq,
                        chain: req.body.chain,
                        type: req.body.geneType
                    }).save().then(async data => {
                        selector.genes.push(data._id);
                        await this.model.findOneAndUpdate({_id: selector._id}, selector);
                    });
                });

                res.status(201).json(selector.genes);
            } else {
    ...
    update = async (req, res) => {
        try {
            ...
            if (objectFromDb[0]._doc.owner === decodedToken.user._id) {
                ...
                res.status(400).json({error: 'This selector does not belong to you.'});
            }
